 ...
 * @version  1.0
 *  */
public class RepertoryPanel extends CommonJPanel {
	...
	private void appendBook() {
		...
		if (Integer.valueOf(this.bookAmount.getText())<1) {
			showWarn("请输入书的数量"); 
			return;
		}
		//获得选中的对象
	...
	private void view() {
		...
		//从数据库中查找
		InRecord r = inRecordService.queryByID(id);
		//设置列表数据对应的集合
	...
	public void setViewDatas() {
		Vector<InRecord> records = (Vector<InRecord>)inRecordService.queryAll(new Date());
	...
	private void buildBooksComboBox() {
		Collection<Book> books = bookService.queryAll();
		for (Book book : books) {
